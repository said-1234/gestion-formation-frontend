...
export class OrganismeComponent implements OnInit {
      ...
      accept: () => {
        if (organisme.organismeId != null) {
          this.organismeService
            .deleteOrganisme(organisme.organismeId)
            .subscribe((data) => {
              this.hideDialog();
              this.ngOnInit();
              if (JSON.stringify(data.message == 'success')) {
                this.organismes = this.organismes.filter(
                  (val) => val.organismeId !== organisme.organismeId
                );
                ...
                this.messageService.add({
                  severity: 'error',
                  summary: 'Probléme de suppression',
                  detail: data.message,
                });
              }
            });
  ...
  saveEditOrganisme(organisme: Organismes) {
    ...
    this.organismeService.addOrganisme(this.OrganismeForm).subscribe(
      (data) => {
        this.messageService.add({
          severity: 'success',
