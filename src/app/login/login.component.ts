...
export class LoginComponent {
  ...
  codePinControle = new FormControl(
    { value: this.data, disabled: true },
    ...
    return this.authService.getUserByLogin(login).subscribe((data) => {
      ...
      if (data != null) {
        ...
        this.messageService.add({
          severity: 'error',
          summary: 'Login error',
          detail: 'Vous devez vérifier votre login',
        });
      }
    });
  }
  ...
  checkCodePassword() {
    ...
    if (!this.hideCode) this.hideCode = true;
    this.userWithPassword = false;
    this.userWithCodepin = false;
  }
  ...
  deleteCodePin() {
    this.codePinControle.setValue(
      this.codePinControle.value.substr(
        0,
        this.codePinControle.value.length - 1
      )
    );
